...
class WithdrawController {
    ...
    async withdrawRequest(req, res) {
        try {
            ...
            if (availableBalance >= amount && availableBalance > 0) {
                let account = await AccountServices.getAccount({ user_id: data.freelancer_id });  //getting account
                if (account) {
                    let details = {
                        withdraw_owner: data.freelancer_id,
                        amount: amount,
                    ...
                    throw new apiError.ValidationError("Account not found", message.NOT_FOUND);
                }
            }
            else {
    ...
    async getAccount(req, res) {
        try {
            ...
            let accDetail = await AccountServices.getAccount(criteria)
            res.status(200).send(ResponseService.success({ bankDetail: accDetail }))
        } catch (e) {
