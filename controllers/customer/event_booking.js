...
class EventBookingController {
    ...
    async getBookedEvent(req, res) {
        try {
            ...
            let mergeEvent = Object.assign({}, bookedEvent.toJSON());
            mergeEvent.arabic_venue = getEvent.arabic_venue;
            mergeEvent.english_venue = getEvent.english_venue;
            mergeEvent.time = getEvent.time;
            mergeEvent.coordinates = getEvent.coordinates;
            mergeEvent.remaining_seats = getEvent.remaining_seats;



            return res.status(200).send(ResponseService.success({ event: mergeEvent }));
