...
class CommentController {
    async addComment(req, res) {
        try {
            ...
            let commentAdded = await CommentService.addComment(details);
            
            let allComment = await CommentService.getAllcomment({ course_id: details.course_id });
    ...
    async getComment(req, res) {
        try {
            ...
            let comment =  await CommentService.getOneComment({ _id: data.id });
    ...
    async replyComment(req, res) {
        try {
            ...
            let replyComment = await CommentService.replyComment({ _id: data.id }, details);
    ...
    async getAllComment(req, res) {
        try {
            ...
            let allComment = await CommentService.getAllcomment({ topic_id: data.topic_id });
    ...
    async getAllCommentsOfCustomer(req, res) {
        try {
            let data = Object.assign({}, req.body);
            let course_id = mongoose.Types.ObjectId(data.course_id);
            let customer_id = mongoose.Types.ObjectId(data.customer_id);

            let getCourse = await CourseService.getCourse({ _id: course_id });
            if (!getCourse) throw new apiError.ValidationError('id', message.COURSE_ID);

            let allComment = await CommentService.getAllcomment({ course_id: course_id, "customer.customer_id":customer_id });
